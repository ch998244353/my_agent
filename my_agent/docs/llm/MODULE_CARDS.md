...
- Public surface: `Agent`, `AgentCapabilities`, `Agent.as_tool`, `Agent.for_code`, `build_coding_agent`, `CodingAgentProfile`, `WorkspaceManifest`, trajectory helpers, `ChatRuntimeConfig`, `ChatRuntime` (`src/agents/agent.py:AgentCapabilities`; `src/agents/agent.py:Agent.as_tool`; `src/agents/coding_agent.py:CodingAgentProfile`; `src/agents/workspace_manifest.py:WorkspaceManifest`; `src/agents/trajectory.py:TrajectoryEvent`; `src/agents/chat_runtime.py:ChatRuntimeConfig`).
...
- Purpose: bind local CLI arguments to `CodingAgentSetup`, run one coding task, persist pending approval state, resume saved approvals from `--resume-state`, print a concise result, and return shell-friendly exit codes (`src/agents/coding_cli.py:run_coding_agent_cli`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`; `src/agents/coding_cli.py:_exit_code_for_result`).
- Owned files: `src/agents/coding_cli.py`, `examples/local_coding_cli.py`, `tests/test_coding_cli.py`.
- Public surface: `ApprovalDecision`, `CodingCliConfig`, `parse_coding_cli_args`, `build_coding_cli_setup`, and `run_coding_agent_cli` through package root lazy exports; config includes workspace/profile/model limits, `--session-json`, `--state-json`, `--resume-state`, approval decisions, `--trajectory-jsonl`, test-command defaults, and verification flags (`--verify-command`, `--verify-after-tool`, `--verify-max-attempts`, `--verify-output-chars`) (`src/agents/coding_cli.py:ApprovalDecision`; `src/agents/coding_cli.py:CodingCliConfig`; `src/agents/coding_cli.py:_build_parser`; `src/agents/__init__.py:__getattr__`).
- Depends on: `build_coding_agent`, `CodingAgentProfile`, `WorkspaceManifest`, `OpenAIResponsesModel`, `JsonSession`, `VerificationPolicy`, `CodingRunStateStore`, `RunContextWrapper`, `RunState`, `resume_agent_loop`, trajectory helpers, and `Agent.run` (`src/agents/coding_agent.py:build_coding_agent`; `src/agents/workspace_manifest.py:WorkspaceManifest`; `src/agents/verification.py:VerificationPolicy`; `src/agents/coding_state.py:CodingRunStateStore`; `src/agents/run_context.py:RunContextWrapper`; `src/agents/run_state.py:RunState`; `src/agents/run_loop.py:resume_agent_loop`; `src/agents/trajectory.py:write_trajectory_jsonl`; `src/agents/agent.py:Agent.run`).
...
- State: no persistent process state of its own; it may pass `JsonSession` into `RunConfig.session`, writes a pending state envelope only when `--state-json` and pending approvals are present, rewrites that path when resume pauses again, deletes it when resume completes, and writes a trajectory file only when `--trajectory-jsonl` is provided. Fresh trajectory output overwrites by default; resume trajectory output appends lifecycle and resumed-run events to the same JSONL path (`src/agents/coding_cli.py:build_coding_cli_setup`; `src/agents/coding_cli.py:_save_pending_state_from_result`; `src/agents/coding_cli.py:_save_or_clear_resumed_state`; `src/agents/coding_cli.py:_write_trajectory_from_result`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`; `src/agents/memory.py:JsonSession`).
- Contracts: exit code `0` means final output, `1` means error or stopped without output, and `2` means pending approval; missing `--state-json` remains compatible but prints that state was not saved. `--resume-state` with no approval decision prints saved pending approval summaries and returns `2` without starting runtime. Fresh and resume pending output prefer the shared `summary` text and fall back to the legacy one-line shape only when no summary exists. Verification summary output is additive and does not override pending approval exit code semantics. Trajectory output records saved-state and resume-decision evidence, but resume execution still comes from the state envelope, not from replaying JSONL (`src/agents/coding_cli.py:_exit_code_for_result`; `src/agents/coding_cli.py:_print_result`; `src/agents/coding_cli.py:_print_verification_summary`; `src/agents/coding_cli.py:_print_pending_approvals_from_envelope`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`; `src/agents/result.py:RunResultBase.pending_approval_summaries`; `src/agents/result.py:RunResultBase.verification_summary`; `src/agents/coding_state.py:CodingRunStateStore.load_envelope`).
- Common edit reasons: new CLI flags, profile selection, result printing, approve/reject resume behavior, package-root export behavior, or module smoke testing (`src/agents/coding_cli.py:_build_parser`; `src/agents/coding_cli.py:_apply_approval_decisions`; `tests/test_coding_cli.py`; `tests/test_tool_approval_pause.py`).
...
## Coding CLI State File

- Purpose: persist a paused local coding CLI run into a stable JSON envelope that `--resume-state` can read for approve/reject resume (`src/agents/coding_state.py:CodingRunStateStore`; `src/agents/coding_state.py:CodingRunStateEnvelope`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`).
- Owned files: `src/agents/coding_state.py`, `tests/test_coding_state.py`; CLI integration lives in `src/agents/coding_cli.py` and `tests/test_coding_cli.py`.
- Public surface: module-level `CodingRunStateStore`, `CodingRunStateEnvelope`, and `CODING_RUN_STATE_ENVELOPE_VERSION`; these are not package-root exports in the current code (`src/agents/coding_state.py:__all__`; `src/agents/__init__.py:__all__`).
- Depends on: `RunResult.to_state`, `WorkspaceManifest.metadata`, CLI config values, and `CodingAgentSetup.workspace` (`src/agents/result.py:RunResultBase.to_state`; `src/agents/workspace_manifest.py:WorkspaceManifest.metadata`; `src/agents/coding_cli.py:CodingCliConfig`; `src/agents/coding_agent.py:CodingAgentSetup`).
- Called by: `_save_pending_state_from_result` after fresh or resumed runs return pending approvals, and by `_run_resumed_coding_agent_cli` when loading `--resume-state` for approval decisions (`src/agents/coding_cli.py:_save_pending_state_from_result`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`; `src/agents/coding_cli.py:_save_or_clear_resumed_state`).
- State: writer creates parent directories and overwrites the target JSON file; reader validates envelope version, object fields, and pending approval shape (`src/agents/coding_state.py:CodingRunStateStore.save_pending_result`; `src/agents/coding_state.py:CodingRunStateStore.load_envelope`).
- Contracts: stores manifest env keys but not env values, stores structured pending approvals with `tool_name`, `call_id`, `arguments`, `reason`, and optional display `summary`, and preserves nested `state` exactly as the run snapshot dict for later resume code (`src/agents/coding_state.py:_workspace_manifest_metadata`; `src/agents/coding_state.py:_pending_approval_to_dict`; `src/agents/coding_state.py:_pending_approval_from_state`).
- Risks: this module validates the file contract but still does not execute approvals itself; execution belongs to `coding_cli.py` rebuilding setup/state and then calling `resume_agent_loop` (`src/agents/coding_cli.py:_run_resumed_coding_agent_cli`; `src/agents/run_loop.py:resume_agent_loop`).
- Evidence: `tests/test_coding_state.py`, `tests/test_coding_cli.py::test_run_coding_agent_cli_writes_state_contract_from_fake_model`.

## Approval Summary Rendering

- Purpose: generate stable, user-readable pending approval text for shell/test commands and patch writes without changing whether a tool requires approval (`src/agents/approval_summaries.py:approval_summary_for_tool_call`; `src/agents/coding_policies.py:ShellCommandPolicy`; `src/agents/coding_policies.py:PatchApprovalPolicy`).
- Owned files: `src/agents/approval_summaries.py`, `tests/test_approval_summaries.py`; integration lives in `src/agents/result.py`, `src/agents/coding_cli.py`, and `src/agents/coding_state.py`.
- Public surface: module-level `approval_summary_for_tool_call(tool_name, call_id, arguments, reason) -> str`; it is not package-root exported in the current code (`src/agents/approval_summaries.py:approval_summary_for_tool_call`; `src/agents/approval_summaries.py:__all__`).
- Depends on: tool call arguments, policy-provided reason text, and `parse_patch()` only for safe display extraction of changed paths and operations (`src/agents/approval_summaries.py:_patch_summary_parts`; `src/agents/patches.py:parse_patch`).
- Called by: `RunResult.pending_approval_summaries`, which then feeds CLI printing and state-file persistence (`src/agents/result.py:RunResultBase.pending_approval_summaries`; `src/agents/coding_cli.py:_print_result`; `src/agents/coding_state.py:CodingRunStateStore.save_pending_result`).
- Contracts: shell/test summaries include `tool`, `call_id`, `command`, `cwd`, `risk`, and `reason`; patch summaries include `tool`, `call_id`, `dry_run`, `changed_paths`, `operations`, `risk`, and `reason`. Summary text must not include env values, and invalid patch text renders `patch parse failed before approval summary` instead of breaking pending display (`src/agents/approval_summaries.py:approval_summary_for_tool_call`; `src/agents/approval_summaries.py:_PATCH_PARSE_FAILED`).
- Risks: summary text is display evidence only; it must not become the policy source of truth or replace `ToolApprovalRequest`, `PendingApprovalSummary`, or state fields needed for resume (`src/agents/contracts.py:ToolApprovalRequest`; `src/agents/result.py:PendingApprovalSummary`; `src/agents/coding_state.py:CodingRunStateEnvelope`).
- Evidence: `tests/test_approval_summaries.py`, `tests/test_result.py::RunResultTestCase.test_pending_approval_summaries_include_approval_summary_text`, `tests/test_coding_cli.py`, `tests/test_coding_state.py`.
...
- Public surface: `WorkspaceManifest` is package-root public; `SafetyDecision`, `ShellCommandPolicy`, and `PatchApprovalPolicy` are module-level policy surfaces used by coding-agent wiring (`src/agents/workspace_manifest.py:WorkspaceManifest`; `src/agents/coding_policies.py:SafetyDecision`; `src/agents/__init__.py:__all__`).
...
- Public surface: `ToolObservation`, `command_result_observation`, `patch_result_observation` through `src/agents/tool_observations.py:__all__`; these are not package-root exports in the current code (`src/agents/tool_observations.py:ToolObservation`; `src/agents/__init__.py:__all__`).
...
- Purpose: convert a completed, paused, or resumed CLI run into a plain JSONL audit file for debugging, teaching, and future eval stages (`src/agents/trajectory.py:trajectory_events_from_result`; `src/agents/trajectory.py:state_saved_event`; `src/agents/trajectory.py:resume_started_event`; `src/agents/trajectory.py:approval_decision_event`; `src/agents/trajectory.py:write_trajectory_jsonl`).
...
- State: no mutable runtime state; writer creates or appends to the target JSONL file only when called. Fresh CLI writes use overwrite mode, while resume writes append `resume_started`, `approval_decision`, resumed runtime events, and optional `state_saved` evidence (`src/agents/trajectory.py:write_trajectory_jsonl`; `src/agents/coding_cli.py:_write_trajectory_from_result`; `src/agents/coding_cli.py:_run_resumed_coding_agent_cli`).
- Contracts: trajectory is separate from tracing and from resume state. It records supported runtime evidence, derives approval rejection from metadata, preserves unknown run items as `runtime_item`, records CLI lifecycle events (`state_saved`, `resume_started`, `approval_decision`), and includes clipped verification summary data on terminal events (`src/agents/trajectory.py:_event_type_from_run_item`; `src/agents/trajectory.py:_event_from_run_item`; `src/agents/trajectory.py:_result_summary_payload`; `src/agents/trajectory.py:_verification_summary_payload`).
- Common edit reasons: new `RunItem.item_type` values, new CLI lifecycle evidence, new summary fields, or later inspector/eval readers (`src/agents/contracts.py:RunItem`; `src/agents/trajectory.py:_result_summary_payload`; `tests/test_trajectory.py`; `tests/test_coding_cli.py`).
...
- Contracts: input guardrails run once only for non-resume runs; repo context builds only for non-resume runs; pending approvals are processed before the main loop on resume. CLI resume prepares approval status first, then delegates execution to this run-loop path (`src/agents/run_loop.py:_run_input_guardrails`; `src/agents/run_loop.py:build_task_repo_context`; `src/agents/run_loop.py:resume_pending_tool_approvals`; `src/agents/coding_cli.py:_run_state_from_state_envelope`).
...
- Public surface: `ChatMessage`, `ToolSpec`, `ToolCall`, `ToolApprovalRequest`, `ModelResponse`, `RunItem`, `StepRecord`, `RunState`, `RunStateSnapshot`, `RunResult`, `ApprovalSnapshot`, `RUN_STATE_SNAPSHOT_SCHEMA_VERSION`, `run_state_snapshot_to_dict`, and `run_state_snapshot_from_dict` (`src/agents/contracts.py:ToolCall`; `src/agents/run_state.py:RunStateSnapshot`; `src/agents/run_state.py:run_state_snapshot_to_dict`; `src/agents/run_state.py:run_state_snapshot_from_dict`; `src/agents/result.py:RunResult`).
...
- Contracts: `RunResult.final_output` aliases `final_answer`; `to_state` returns a schema-versioned JSON-safe snapshot dict, exports pending approvals even when no context wrapper is attached, normalizes model/run item payloads through `_json_safe_value`, and exposes `pending_approval_summaries` with old fields plus display `summary` (`src/agents/result.py:RunResultBase.final_output`; `src/agents/result.py:RunResultBase.to_state`; `src/agents/result.py:_json_safe_value`; `src/agents/result.py:RunResultBase.pending_approval_summaries`; `src/agents/result.py:PendingApprovalSummary`).
...
- Risks: unknown payloads are JSON-safe but may only preserve `{type, repr}` unless `_run_item_payload_to_state` has a semantic converter; snapshot helpers are not a CLI file format by themselves (`src/agents/result.py:_run_item_payload_to_state`; `src/agents/result.py:_json_safe_value`; `src/agents/run_state.py:run_state_snapshot_to_dict`).
- Evidence: `tests/test_contracts.py`, `tests/test_result.py`, `tests/test_run_state.py`, `tests/test_run_context_approvals.py`.

## Verification and Tests

- Purpose: optionally run configured verification commands after selected tools, expose result summaries, and let the coding CLI configure/print that loop (`src/agents/verification.py:VerificationPolicy`; `src/agents/run_recording.py:run_verification_after_tool`; `src/agents/result.py:RunResultBase.verification_summary`; `src/agents/coding_cli.py:build_coding_cli_setup`).
- Owned files: `src/agents/verification.py`, verification paths in `src/agents/run_recording.py`, CLI wiring in `src/agents/coding_cli.py`, and behavior coverage in `tests/test_verification.py`, `tests/test_verification_loop.py`, and `tests/test_coding_cli.py`.
...
- Called by: run loop after each executed non-handoff tool; local coding CLI builds the policy from `--verify-*` flags and appends explicit verification commands to the manifest test-command allowlist (`src/agents/run_loop.py:_run_agent_loop_impl`; `src/agents/run_recording.py:run_verification_after_tool`; `src/agents/coding_cli.py:build_coding_cli_setup`; `src/agents/workspace_manifest.py:WorkspaceManifest`).
...
- Contracts: verification runs only if policy exists, runner exists, commands enabled, triggering tool is in `auto_after_tools`, and max attempts not reached. Without CLI `--verify-command`, `RunConfig.verification` remains unset and no automatic verification runs. Verification failure is observation/evidence, not an exception mechanism (`src/agents/verification.py:VerificationPolicy.should_run_after_tool`; `src/agents/run_recording.py:run_verification_after_tool`; `src/agents/coding_cli.py:build_coding_cli_setup`; `src/agents/verification.py:VerificationResult.to_observation`).
...
- Evidence: `tests/test_verification.py`, `tests/test_verification_loop.py`, `tests/test_coding_cli.py`.
